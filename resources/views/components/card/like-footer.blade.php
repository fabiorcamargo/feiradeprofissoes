    ...
    <div class="relative flex h-16 items-center justify-between">
        <div class="absolute inset-y-0 left-0 flex items-center">
            <div class="relative flex">
                <div>
                    <a href="javascript:likeClick{{$l}}({{ $l }})" class="block">
                        <svg class="heart-icon {{Cookie::get("c_like$l")}} ic{{$l}}" width="25" height="25" viewBox="0 0 106 97" fill="none"
                    ...
                    <p class="inline-block ml-8 mt-4 text-gray-500 dark:text-gray-500 text-md leading-relaxed">
                        <span class="number-of-likes cli{{$l}} text-left">{{ $q }} </span>
                    </p>
                </div>
            </div>
        </div>
...
<script>
    const likeBtn{{$l}} = document.querySelector('.ic{{$l}}');
const numberOfLikesElement{{$l}} = document.querySelector('.cli{{$l}}');

let numberOfLikes{{$l}} = Number.parseInt(numberOfLikesElement{{$l}}.textContent, 10);
let isLiked{{$l}} = "{{Cookie::get("c_like$l")}}" ;

// Functions

const likeClick{{$l}} = (id) => {
  if (!isLiked{{$l}}) {
    likeBtn{{$l}}.classList.add('isLiked');
    numberOfLikes{{$l}}++;
    numberOfLikesElement{{$l}}.textContent = numberOfLikes{{$l}};
    isLiked{{$l}} = !isLiked{{$l}};
    like_up(id);
  } else {
    likeBtn{{$l}}.classList.remove('isLiked');
    numberOfLikes{{$l}}--;
    numberOfLikesElement{{$l}}.textContent = numberOfLikes{{$l}};
    isLiked{{$l}} = !isLiked{{$l}};
    like_down(id);
    ...
    function like_down(id){
        console.log(id);
        httpGet('like/down/'+id);
    };
